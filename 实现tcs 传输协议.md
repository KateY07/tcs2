...
主控端尚未固定身份时，允许先读取并验证 ServerHello 的签名，结束该探测连接，再展示主机指纹供用户通过独立可信渠道核对。明确确认后保存公钥，并重新按完整握手建立连接；重新连接仍须严格比对已确认公钥。探测不发送 ClientAuth 或业务数据。此引导过程不改变 TCS/1 的消息格式，也不要求回执文件。
